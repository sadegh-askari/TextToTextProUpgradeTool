...
namespace TextToTMPNamespace
{
    public partial class TextToTMPWindow : EditorWindow, IHasCustomMenu
    {
        ...
        {
            InitializeAndCollectReferences,
            UpgradeScripts,
            PendingUpgradeScriptsCompletion,
            UpgradeComponents,
            UpdateReferences,
            Finished
        };
        ...
        private const string DONT_CLOSE_SCENES_WARNING =
            "Scene(s) inside the 'Assets & Scenes To Upgrade' list will automatically be opened in order to upgrade the GameObjects inside them. You MUST NOT close these scenes until all the steps are completed!";

        private const string BACKUP_PROJECT_WARNING =
            "Beyond this point, you can't undo your actions or go back. You are strongly recommended to backup your project before proceeding!";

        private const string DONT_CLOSE_WINDOW_WARNING =
            "After this step, you MUST NOT close this window until all the steps are completed! Otherwise, you won't be able to restore the references to the upgraded components.";

        private const string FIX_COMPILATION_ERRORS_WARNING =
            "If there are any compiler errors in the Console, then it means that some script(s) couldn't be upgraded completely. You should fix those errors manually before proceeding.";
        ...
        [MenuItem("Window/Upgrade TMP to TextPro")]
        private static void Init()
        {
            TextToTMPWindow window = GetWindow<TextToTMPWindow>();
            window.titleContent = new GUIContent("TMP to TextPro");
        ...
        void IHasCustomMenu.AddItemsToMenu(GenericMenu menu)
        {
            if (stage == UpgradeStage.InitializeAndCollectReferences)
            {
                ...
                {
                    string savePath =
                        EditorUtility.SaveFilePanel("Save Settings To", "Library", "TextToTMP Settings", "json");
                ...
                menu.AddItem(new GUIContent("Load Settings"), false, () =>
                {
                    ...
                    if (!string.IsNullOrEmpty(loadPath))
                    {
                        ...
                        if (!saveData
                                .saveDataValidityCheck) // If a random JSON file is loaded, it won't overwrite all the settings by mistake
        ...
        private void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Text To TMP can't work while there are unsaved changes in the Prefab stage.",
                    MessageType.Error);
                return;
            }
#endif
            ...
            if (stage == UpgradeStage.InitializeAndCollectReferences)
            {
                ...
                for (int i = 0; i < upgradeTargets.Count; i++)
                {
                    ...
                    upgradeTargets[i] =
                        EditorGUILayout.ObjectField(GUIContent.none, upgradeTargets[i], typeof(Object), false);
                ...
                EditorGUILayout.HelpBox(
                    "You can add folder(s) to this list. Their contents will be included in the upgrade process.",
                    MessageType.Info);
                    ...
                    GUILayoutUtility.GetLastRect().Contains(ev.mousePosition))
                {
                    ...
                    if (ev.type == EventType.DragPerform)
                    {
                        ...
                        if (draggedObjects != null)
                        {
                            ...
                            for (int i = 0, count = Mathf.Max(draggedUnityFonts.Count, draggedTMPFonts.Count);
                                 i < count;
                                 i++)
                            {
                ...
                for (int i = 0; i < fontUpgrades.Count; i++)
                {
                    ...
                    if (i == 0)
                        ...
                        GUILayout.Label(
                            "TMP Font for: " + (fontUpgrades[i].unityFont
                                ? fontUpgrades[i].unityFont.name
                                : "<Font not assigned>"), boldWordWrappedLabel);
                    ...
                    if (i != 0)
                        fontUpgrades[i].unityFont =
                            EditorGUILayout.ObjectField("Unity Font", fontUpgrades[i].unityFont, typeof(Font),
                                false) as Font;

                    EditorGUI.BeginChangeCheck();
                    fontUpgrades[i].tmpFont =
                        EditorGUILayout.ObjectField("TMP Font", fontUpgrades[i].tmpFont, typeof(TMP_FontAsset), false)
                            as TMP_FontAsset;
                    if (EditorGUI.EndChangeCheck())
                    {
                        fontUpgrades[i].tmpFontMaterialDefault =
                            fontUpgrades[i].tmpFont ? fontUpgrades[i].tmpFont.material : null;
                    ...
                    fontUpgrades[i].tmpFontMaterialDefault = EditorGUILayout.ObjectField("TMP Font Default Material",
                        fontUpgrades[i].tmpFontMaterialDefault, typeof(Material), false) as Material;
                    fontUpgrades[i].tmpFontMaterialShadow = EditorGUILayout.ObjectField("TMP Font Shadow Material",
                        fontUpgrades[i].tmpFontMaterialShadow, typeof(Material), false) as Material;
                    fontUpgrades[i].tmpFontMaterialOutline = EditorGUILayout.ObjectField("TMP Font Outline Material",
                        fontUpgrades[i].tmpFontMaterialOutline, typeof(Material), false) as Material;
                    fontUpgrades[i].tmpFontMaterialShadowAndOutline =
                        EditorGUILayout.ObjectField("TMP Font Shadow & Outline Material",
                            fontUpgrades[i].tmpFontMaterialShadowAndOutline, typeof(Material), false) as Material;
                }

                EditorGUILayout.Space();

                if (GUILayout.Button("START", GL_HEIGHT_30) && (!upgradeScenes || EditorUtility.DisplayDialog("Warning",
                        DONT_CLOSE_SCENES_WARNING.Replace("&", "and"), "Got it!", "Cancel")))
                {
            ...
            else if (stage == UpgradeStage.UpgradeScripts)
            {
                ...
                GUILayout.Label(
                    "Text, InputField, Dropdown and TextMesh terms used inside these scripts will be upgraded to their TextMesh Pro variants:",
                    boldWordWrappedLabel);
                ...
                GUILayout.Label(
                    "TextMesh Pro's Assembly Definition File (if exists) will be added to these Assembly Definition Files' 'Assembly Definition References' list:",
                    boldWordWrappedLabel);
...
#endif
                if (GUILayout.Button("UPGRADE SCRIPTS", GL_HEIGHT_30) &&
                    EditorUtility.DisplayDialog("Warning", BACKUP_PROJECT_WARNING, "Got it!", "Cancel") &&
                    EditorUtility.DisplayDialog("Warning", DONT_CLOSE_WINDOW_WARNING, "Got it!", "Cancel"))
                {
                    AssetDatabase.SaveAssets();
                    UpgradeScripts();
                ...
                if (GUILayout.Button("DON'T UPGRADE SCRIPTS", GL_HEIGHT_30) &&
                    EditorUtility.DisplayDialog("Warning", BACKUP_PROJECT_WARNING, "Got it!", "Cancel") &&
                    EditorUtility.DisplayDialog("Warning", DONT_CLOSE_WINDOW_WARNING, "Got it!", "Cancel"))
            ...
            else if (stage == UpgradeStage.PendingUpgradeScriptsCompletion)
            {
                ...
                if (GUILayout.Button("NEXT STEP", GL_HEIGHT_30) &&
                    EditorUtility.DisplayDialog("Warning", FIX_COMPILATION_ERRORS_WARNING, "Got it!", "Cancel"))
                    SwitchStage(UpgradeStage.UpgradeComponents);
            ...
            else if (stage == UpgradeStage.UpgradeComponents)
            {
                ...
                GUILayout.Label(
                    "Text, InputField, Dropdown and TextMesh component(s) on the following assets will be upgraded to their TextMesh Pro variants:",
                    boldWordWrappedLabel);
                ...
                GUILayout.Label("The same component(s) inside these Scenes will also be upgraded:",
                    boldWordWrappedLabel);
                ...
                {
                    GUILayout.Label(
                        "When a Text's 'Horizontal Overflow' is set to 'Overflow' and 'Vertical Overflow' is set to 'Truncate', its behaviour will change when it is upgraded to TextMesh Pro:",
                        boldWordWrappedLabel);

                    alwaysUseOverflowForNonWrappingTexts = !WordWrappingToggleLeft(
                        "Set TextMesh Pro's 'Overflow' value to 'Truncate': text will truncate both horizontally and vertically, i.e. it won't overflow horizontally",
                        !alwaysUseOverflowForNonWrappingTexts);
                    alwaysUseOverflowForNonWrappingTexts = WordWrappingToggleLeft(
                        "Set TextMesh Pro's 'Overflow' value to 'Overflow': text will overflow both horizontally and vertically, i.e. it won't truncate vertically",
                        alwaysUseOverflowForNonWrappingTexts);
            ...
            else if (stage == UpgradeStage.UpdateReferences)
            {
                ...
                GUILayout.Label(
                    "For objects whose variables were referencing the upgraded Text, InputField, Dropdown and TextMesh components, these variables will now refer to the components' TextMesh Pro variants.",
                    boldWordWrappedLabel);
            ...
            else if (stage == UpgradeStage.Finished)
            {
                ...
                GUILayout.Label(
                    "You can now safely close this window and close the scenes that were opened during the upgrade process (if any).",
                    boldWordWrappedLabel);
        ...
        private void Initialize()
        {
            ...
            foreach (string assetPath in upgradeTargetsSet)
            {
                ...
                {
                    if (upgradeScenes &&
                        typeof(SceneAsset).IsAssignableFrom(AssetDatabase.GetMainAssetTypeAtPath(assetPath)))
                    {
...
#endif
                else if (AssetDatabase.LoadAssetAtPath<GameObject>(assetPath) ||
        ...
        private TMP_FontAsset GetCorrespondingTMPFontAsset(Font font, Component source, out Material fontMaterial)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < modifierComponents.Length; i++)
                {
                    if (modifierComponents[i] && (modifierComponents[i].GetType() == typeof(Shadow) ||
                                                  modifierComponents[i].GetType() == typeof(Outline)))
                    {
                        stringBuilder.Append("Removing ").Append(modifierComponents[i].GetType().Name)
                            .Append(" component from ").AppendLine(GetPathOfObject(source.transform));
