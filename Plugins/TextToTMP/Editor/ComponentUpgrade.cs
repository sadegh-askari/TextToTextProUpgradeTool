...
namespace TextToTMPNamespace
{
    public partial class TextToTMPWindow
    {
        ...
        private readonly List<PrefabInstancesRemovedComponent> upgradedComponentsToRemoveInPrefabInstances =
            new List<PrefabInstancesRemovedComponent>();

        private FieldInfo unityEventPersistentCallsField,
            unityEventPersistentCallsListField,
            unityEventPersistentCallTargetField;
        ...
        private void UpgradeComponents()
        {
            ...
            try
            {
                ...
                {
                    EditorUtility.DisplayProgressBar("Upgrading components in assets...", asset,
                        (float)progressCurrent / progressTotal);
                    progressCurrent++;

                    if (asset.EndsWith(".prefab", StringComparison.OrdinalIgnoreCase) &&
                        prefabsToUpgradePaths.Add(asset))
                    {
                ...
                if (prefabCyclicReferenceCheckerMethod == null)
                    prefabCyclicReferenceCheckerMethod = typeof(PrefabUtility).GetMethod(
                ...
                for (int i = 0; i < prefabsToUpgrade.Count - 1; i++)
                {
                    ...
                    for (int j = i + 1; j < prefabsToUpgrade.Count; j++)
                    {
                        ...
                        if ((bool)prefabCyclicReferenceCheckerMethod.Invoke(null,
                                new object[] { prefabsToUpgrade[j], prefabsToUpgrade[swapIndex] }))
                            swapIndex = j;
                    }
                ...
                {
                    EditorUtility.DisplayProgressBar("Upgrading components in prefabs...",
                        AssetDatabase.GetAssetPath(prefab), (float)progressCurrent / progressTotal);
                    progressCurrent++;
                ...
                {
                    EditorUtility.DisplayProgressBar("Upgrading components in scenes...", scene,
                        (float)progressCurrent / progressTotal);
                    progressCurrent++;
                ...
                foreach (InputFieldProperties modifiedPrefabInstance in modifiedInputFieldPrefabInstances)
                    ApplyPrefabInstanceModifications(modifiedPrefabInstance,
                        (tmp) => PasteInputFieldProperties(tmp, modifiedPrefabInstance), ref progressCurrent,
                        progressTotal);
                foreach (DropdownProperties modifiedPrefabInstance in modifiedDropdownPrefabInstances)
                    ApplyPrefabInstanceModifications(modifiedPrefabInstance,
                        (tmp) => PasteDropdownProperties(tmp, modifiedPrefabInstance), ref progressCurrent,
                        progressTotal);
                // foreach (TextMeshProperties modifiedPrefabInstance in modifiedTextMeshProPrefabInstances)
                //     ApplyPrefabInstanceModifications(modifiedPrefabInstance,
...
#else
				prefab =
 PrefabUtility.ReplacePrefab( prefabInstanceRoot, prefab, ReplacePrefabOptions.ConnectToPrefab );
#endif
            }
            finally
            {
            ...
            foreach (PrefabInstancesRemovedComponent removedComponentHolder in
                     upgradedComponentsToRemoveInPrefabInstances)
            {
                ...
                foreach (GameObject prefabInstance in removedComponentHolder.removedPrefabInstances)
                {
                    ...
                    foreach (Component component in prefabInstance.GetComponents(removedComponentHolder
                                 .UpgradedComponentType))
                    {
                        if (!component)
                            continue;
                        ...
                        {
                            stringBuilder.Append("Removing ").Append(component.GetType().Name).Append(" from ")
                                .Append(GetPathOfObject(component.transform))
                                .AppendLine(" since its legacy version was also removed as prefab override");
        ...
        private void UpgradeGameObjectRecursively(GameObject go, GameObject prefab)
        {
            ...
            catch (Exception e)
            {
                if (!e.Data.Contains("LoggedHierarchy"))
                {
                    ...
                    Debug.LogError("Error while upgrading components of: " + GetPathOfObject(go.transform),
                        prefab ? prefab.transform.root.gameObject : go);
                }
        ...
        private void PasteInputFieldProperties(TMP_InputField tmp, InputFieldProperties properties)
        {
            ...
            tmp.textComponent = properties.textComponentGameObject
                ? properties.textComponentGameObject.GetComponent<TextMeshProUGUI>()
                : null;
            tmp.placeholder = properties.placeholderGameObject
                ? properties.placeholderGameObject.GetComponent<Graphic>()
                : null;
            ...
            try
            {
                if (properties.hasCaretColor) tmp.caretColor = properties.caretColor;
            }
            catch
            {
            }
            ...
            if (tmp.lineType == properties.lineType)
                tmp.lineType =
                    (TMP_InputField.LineType)(((int)properties.lineType + 1) %
                                              3); // lineType adjusts Text's enableWordWrapping setting but only if the lineType value is different
            ...
            {
                stringBuilder.Append("Upgrading InputField.OnSubmit event using ")
                    .Append(typeof(TMP_InputFieldOnSubmitEvent).Name).Append(" component: ")
                    .AppendLine(GetPathOfObject(tmp.transform));
                TMP_InputFieldOnSubmitEvent bridgeComponent = tmp.GetComponent<TMP_InputFieldOnSubmitEvent>();
                if (bridgeComponent == null &&
                    ((IList)unityEventPersistentCallsListField.GetValue(properties.onSubmit.persistentCalls)).Count > 0)
        ...
        private void PasteDropdownProperties(TMP_Dropdown tmp, DropdownProperties properties)
        {
            ...
            tmp.captionText = properties.captionTextGameObject
                ? properties.captionTextGameObject.GetComponent<TextMeshProUGUI>()
                : null;
            tmp.itemText = properties.itemTextGameObject
                ? properties.itemTextGameObject.GetComponent<TextMeshProUGUI>()
                : null;
        ...
        private void OnComponentIsBeingUpgraded<T>(T component, T prefabComponent) where T : Component
        {
            if (prefabComponent)
            {
                PrefabInstancesRemovedComponent removedComponentHolder =
        ...
        private void CreateInputFieldViewport(TMP_InputField tmp)
        {
            ...
            else
            {
                try
                {
                    viewport = (RectTransform)new GameObject(TMP_INPUT_FIELD_TEXT_AREA_NAME, typeof(RectTransform),
                        typeof(RectMask2D)).transform;
                    ...
                    viewport.GetComponent<RectMask2D>().padding = new Vector4(2f - viewport.offsetMin.x,
                        1f - viewport.offsetMin.y, 2f + viewport.offsetMax.x, 2f + viewport.offsetMax.y);
#endif
                            ...
                            if (child.parent == viewport)
                            {
                                ...
                                if ((child.anchorMax - child.anchorMin).sqrMagnitude > Mathf.Epsilon)
                                {
                                    ...
                                    if (rectTransformAnchorsSetterMethod == null)
                                        rectTransformAnchorsSetterMethod = typeof(Editor).Assembly
                                            .GetType("UnityEditor.RectTransformEditor").GetMethod("SetAnchorSmart",
                                                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static,
                                                null,
                                                new Type[]
                                                {
                                                    typeof(RectTransform), typeof(float), typeof(int), typeof(bool),
                                                    typeof(bool), typeof(bool)
                                                }, null);

                                    rectTransformAnchorsSetterMethod.Invoke(null,
                                        new object[] { child, 0, 0, false, true, true });
                                    rectTransformAnchorsSetterMethod.Invoke(null,
                                        new object[] { child, 1, 0, true, true, true });
                                    rectTransformAnchorsSetterMethod.Invoke(null,
                                        new object[] { child, 0, 1, false, true, true });
                                    rectTransformAnchorsSetterMethod.Invoke(null,
                                        new object[] { child, 1, 1, true, true, true });
                                }
        ...
        private void ApplyPrefabInstanceModifications<LegacyComponentType, UpgradedComponentType>(
            ComponentProperties<LegacyComponentType, UpgradedComponentType> properties,
            Action<UpgradedComponentType> applyModificationsFunction, ref int progressCurrent, int progressTotal)
            where LegacyComponentType : Component where UpgradedComponentType : Component
        {
            EditorUtility.DisplayProgressBar("Applying prefab instance modifications...",
                properties.gameObject ? GetPathOfObject(properties.gameObject.transform) : "Missing",
                (float)progressCurrent / progressTotal);
            progressCurrent++;

            if (!properties.gameObject)
            {
                stringBuilder.Append("<b>Prefab instance with ").Append(typeof(LegacyComponentType).Name)
                    .AppendLine(" modifications is missing</b>");
                return;
            }
            ...
            if (!tmp)
            {
                stringBuilder.Append("<b>Prefab instance with modifications is missing its upgraded ")
                    .Append(typeof(UpgradedComponentType).Name).AppendLine(" component</b>");
                return;
            }

            stringBuilder.Append("Applying prefab instance modifications to upgraded component: ")
                .Append(GetPathOfObject(tmp.transform)).Append(".").AppendLine(typeof(UpgradedComponentType).Name);
        ...
        private TMP_InputField.CharacterValidation GetTMPCharacterValidation(
        ...
        private TextOverflowModes GetTMPVerticalOverflow(VerticalWrapMode verticalOverflow,
            HorizontalWrapMode horizontalOverflow)
        {
            ...
            return verticalOverflow == VerticalWrapMode.Overflow
        ...
        private UnityEventProperties CopyUnityEvent(UnityEventBase target)
        {
            ...
            if (unityEventPersistentCallsField == null)
            {
                ...
                unityEventPersistentCallsField = type.GetField("m_PersistentCalls", flags) ??
                                                 type.GetField("m_PersistentListeners", flags);
                if (unityEventPersistentCallsField != null)
                {
                    type = unityEventPersistentCallsField.FieldType;
                    unityEventPersistentCallsListField =
                        type.GetField("m_Calls", flags) ?? type.GetField("m_Listeners", flags);
                    if (unityEventPersistentCallsListField != null)
                    {
                        type = unityEventPersistentCallsListField.FieldType.GetGenericArguments()[0];
                        unityEventPersistentCallTargetField =
                            type.GetField("m_Target", flags) ?? type.GetField("instance", flags);
                    }
                }
            ...
            unityEventPersistentCallsListField.SetValue(persistentCalls,
                Activator.CreateInstance(unityEventPersistentCallsListField.FieldType,
                    unityEventPersistentCallsListField.GetValue(unityEventPersistentCallsField.GetValue(target))));
            ...
            UnityEventProperties.TargetType[] targetTypes =
                new UnityEventProperties.TargetType[target.GetPersistentEventCount()];
        ...
        private void PasteUnityEvent(UnityEventBase target, UnityEventProperties unityEventProperties)
        {
            ...
            {
                IList persistentCallsList =
                    (IList)unityEventPersistentCallsListField.GetValue(unityEventProperties.persistentCalls);
                for (int i = 0; i < unityEventProperties.targetTypes.Length; i++)
                {
                    ...
                    {
                        case UnityEventProperties.TargetType.Text:
                            component = gameObject.GetComponent<TextMeshProUGUI>();
                            break;
                        case UnityEventProperties.TargetType.InputField:
                            component = gameObject.GetComponent<TMP_InputField>();
                            break;
                        case UnityEventProperties.TargetType.Dropdown:
                            component = gameObject.GetComponent<TMP_Dropdown>();
                            break;
                        case UnityEventProperties.TargetType.TextMesh:
                            component = gameObject.GetComponent<TextMeshPro>();
                            break;
                    }
                ...
                stringBuilder.AppendLine(
                    "<b>Couldn't paste UnityEvent because it became null (it can happen on Unity 2019.2 or earlier if a script was modified during the upgrade process)</b>");
        }
    }
}