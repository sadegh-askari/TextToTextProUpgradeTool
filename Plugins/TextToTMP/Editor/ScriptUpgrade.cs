...
";
        ...
        {
#if UNITY_2017_3_OR_NEWER
            textMeshProAssemblyDefinitionFilePath =
                CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName("TextPro");
            if (!string.IsNullOrEmpty(textMeshProAssemblyDefinitionFilePath))
                textMeshProAssemblyDefinitionName = "TextPro";
            else
            {
                string[] tmpScriptGuid = AssetDatabase.FindAssets("TextPro t:MonoScript");
                if (tmpScriptGuid != null && tmpScriptGuid.Length > 0)
                {
                    textMeshProAssemblyDefinitionName =
                        CompilationPipeline.GetAssemblyNameFromScriptPath(
                            AssetDatabase.GUIDToAssetPath(tmpScriptGuid[0]));
            ...
            try
            {
                ...
                {
                    EditorUtility.DisplayProgressBar("Upgrading scripts...", script,
                ...
                {
                    EditorUtility.DisplayProgressBar("Upgrading Assembly Definition Files...", assemblyDefinitionFile,
                        (float)progressCurrent / progressTotal);
                    progressCurrent++;
        ...
        private void UpgradeScript(string scriptPath)
        {
            ...
            for (int i = 0; i < tokenReplacementsVariableDeclarations.Length; i += 2)
            {
                ...
                scriptModified = scriptModified.Replace("public " + tokenReplacementsVariableDeclarations[i] + " ",
                    "public " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("private " + tokenReplacementsVariableDeclarations[i] + " ",
                    "private " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("protected " + tokenReplacementsVariableDeclarations[i] + " ",
                    "protected " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("internal " + tokenReplacementsVariableDeclarations[i] + " ",
                    "internal " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("static " + tokenReplacementsVariableDeclarations[i] + " ",
                    "static " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("readonly " + tokenReplacementsVariableDeclarations[i] + " ",
                    "readonly " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("virtual " + tokenReplacementsVariableDeclarations[i] + " ",
                    "virtual " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("volatile " + tokenReplacementsVariableDeclarations[i] + " ",
                    "volatile " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("sealed " + tokenReplacementsVariableDeclarations[i] + " ",
                    "sealed " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("partial " + tokenReplacementsVariableDeclarations[i] + " ",
                    "partial " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("extern " + tokenReplacementsVariableDeclarations[i] + " ",
                    "extern " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                // Variable declarations without modifier (1st: indentation with tabs, 2nd: indentation with spaces)
                scriptModified = scriptModified.Replace("\t" + tokenReplacementsVariableDeclarations[i] + " ",
                    "\t" + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace("  " + tokenReplacementsVariableDeclarations[i] + " ",
                    "  " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                // Arrays
                scriptModified = scriptModified.Replace("\t" + tokenReplacementsVariableDeclarations[i] + "[]",
                ...
                scriptModified = scriptModified.Replace(" " + tokenReplacementsVariableDeclarations[i] + "[]",
                    " " + tokenReplacementsVariableDeclarations[i + 1] + "[]");
                scriptModified = scriptModified.Replace(" new " + tokenReplacementsVariableDeclarations[i] + "[",
                    " new " + tokenReplacementsVariableDeclarations[i + 1] + "[");
                // Generics
                scriptModified = scriptModified.Replace("<" + tokenReplacementsVariableDeclarations[i] + ">",
                    "<" + tokenReplacementsVariableDeclarations[i + 1] + ">");
                scriptModified = scriptModified.Replace("< " + tokenReplacementsVariableDeclarations[i] + " >",
                ...
                scriptModified = scriptModified.Replace("( " + tokenReplacementsVariableDeclarations[i] + " )",
                    "( " + tokenReplacementsVariableDeclarations[i + 1] + " )");
                scriptModified = scriptModified.Replace(" is " + tokenReplacementsVariableDeclarations[i] + " ",
                    " is " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace(" as " + tokenReplacementsVariableDeclarations[i] + " ",
                    " as " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                scriptModified = scriptModified.Replace(" is " + tokenReplacementsVariableDeclarations[i] + ")",
                    " is " + tokenReplacementsVariableDeclarations[i + 1] + ")");
                scriptModified = scriptModified.Replace(" as " + tokenReplacementsVariableDeclarations[i] + ")",
                    " as " + tokenReplacementsVariableDeclarations[i + 1] + ")");
                ...
                scriptModified = scriptModified.Replace("this " + tokenReplacementsVariableDeclarations[i] + " ",
                    "this " + tokenReplacementsVariableDeclarations[i + 1] + " ");
                // Instantiation
                scriptModified = scriptModified.Replace("new " + tokenReplacementsVariableDeclarations[i] + "(",
                    "new " + tokenReplacementsVariableDeclarations[i + 1] + "(");
                scriptModified = scriptModified.Replace("new " + tokenReplacementsVariableDeclarations[i] + " (",
            ...
            for (int i = 0; i < tokenReplacementsWithTMPChecks.Length; i += 2)
            {
                ...
                while ((index = scriptModified.IndexOf(tokenReplacementsWithTMPChecks[i], index + 1)) >= 0)
                {
                    if (index == 0 || scriptModified[index - 1] != '_')
                        scriptModified = string.Concat(scriptModified.Substring(0, index),
                            tokenReplacementsWithTMPChecks[i + 1],
                            scriptModified.Substring(index + tokenReplacementsWithTMPChecks[i].Length));
                }
            }
            ...
            for (int i = 0; i < tokenReplacementsExtensionFunctions.Length; i += 3)
            {
                ...
                while ((index = scriptModified.IndexOf(tokenReplacementsExtensionFunctions[i], index + 1)) >= 0)
                {
                    ...
                    if (endIndex >= 0)
                    {
                        ...
                        scriptModified = string.Concat(scriptModified.Substring(0, index),
                            tokenReplacementsExtensionFunctions[i + 1],
                            scriptModified.Substring(functionParametersStartIndex, functionParametersLength),
                            tokenReplacementsExtensionFunctions[i + 2], scriptModified.Substring(endIndex));
        ...
        private void AddExtensionFunctionsToScript(string scriptPath, ref string scriptContents)
        {
            ...
            scriptContents = scriptContents.Insert(extensionFunctionsInsertIndex,
                string.Format(EXTENSION_FUNCTIONS_NAMESPACE_DECLARATION, namespaceGuid) + EXTENSION_FUNCTIONS_BODY);
        }
        ...
        private void UpgradeAssemblyDefinitionFile(string path)
        {
            ...
            AssemblyDefinitionFileContents asmFile =
                JsonUtility.FromJson<AssemblyDefinitionFileContents>(File.ReadAllText(path));
            if (asmFile.references != null)
            {
                for (int i = 0; i < asmFile.references.Length; i++)
                {
                    ...
                    string assemblyPath =
                        CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyReference(asmFile.references[i]);
#else
					string assemblyPath =
 CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName( asmFile.references[i] );
#endif
        ...
        private void AddScriptToUpgrade(string scriptPath)
        {
            ...
            if (scriptPath.StartsWith("Packages/com.unity.textmeshpro", StringComparison.OrdinalIgnoreCase) ||
                scriptPath.StartsWith("Packages/com.hexagon.textpro", StringComparison.OrdinalIgnoreCase) ||
                scriptPath.StartsWith("Packages/com.unity.ui", StringComparison.OrdinalIgnoreCase) ||
                scriptPath.StartsWith("Packages/com.unity.ugui", StringComparison.OrdinalIgnoreCase))
                return;
