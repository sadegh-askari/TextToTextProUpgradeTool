...
namespace TextToTMPNamespace
{
    public partial class TextToTMPWindow
    {
        ...
        private void CollectReferences()
        {
            ...
            MethodInfo fieldInfoGetterMethod = typeof(Editor).Assembly.GetType("UnityEditor.ScriptAttributeUtility")
 ...
 typeof( Editor ).Assembly.GetType( "UnityEditor.ScriptAttributeUtility" ).GetMethod( "GetFieldInfoFromProperty", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static );
            ...
            try
            {
                ...
                {
                    EditorUtility.DisplayProgressBar("Collecting references from assets...", asset,
                ...
                {
                    EditorUtility.DisplayProgressBar("Collecting references from scenes...", scene,
                        (float)progressCurrent / progressTotal);
                    progressCurrent++;
            ...
            try
            {
                foreach (PendingReferenceUpdate reference in pendingReferenceUpdates)
                {
                    EditorUtility.DisplayProgressBar("Updating references...",
                        reference.source ? reference.source.name : "Object", (float)progressCurrent / progressTotal);
                    progressCurrent++;

                    if (!reference.source)
                    {
                        reference.source =
                            ConvertReferencePathToObject(reference.sourcePath, Type.GetType(reference.sourceType));
                        if (!reference.source)
                        {
                    ...
                    for (int j = 0; j < reference.propertyPaths.Length; j++)
                    {
                        ...
                        if (!target)
                        {
                            target = ConvertReferencePathToObject(reference.targetPaths[j],
                        ...
                        switch (reference.targetTypes[j])
                        {
                            case PendingReferenceUpdate.TargetType.Text:
                                target = ((GameObject)target).GetComponent<TextMeshProUGUI>();
                                break;
                            case PendingReferenceUpdate.TargetType.InputField:
                                target = ((GameObject)target).GetComponent<TMP_InputField>();
                                break;
                            case PendingReferenceUpdate.TargetType.Dropdown:
                                target = ((GameObject)target).GetComponent<TMP_Dropdown>();
                                break;
                            case PendingReferenceUpdate.TargetType.TextMesh:
                                target = ((GameObject)target).GetComponent<TextMeshPro>();
                                break;
        ...
        private void CollectReferencesFromObject(Object obj)
        {
            ...
            if (iterator.Next(true))
            {
                ...
                do
                {
                    ...
                    if (isVisible)
                        iteratingVisible =
                            iteratorVisible.NextVisible(iteratorVisible.propertyType == SerializedPropertyType.Generic);
                    else
                    {
                        Type propFieldType;
        ...
        private Object ConvertReferencePathToObject(string referencePath, Type referenceType)
        {
            ...
            string hierarchyPath = referencePath.Length > (pathSplitIndex + REFERENCE_PATH_SEPARATOR.Length)
            ...
            else if (!typeof(Component).IsAssignableFrom(referenceType) &&
            ...
            if (rootAsset is SceneAsset)
            {
                ...
                {
                    Object result =
                        FindObjectInHierarchyRecursively(sceneRoot[i].transform, pathComponents, 0, referenceType);
                    if (result)
                        return result;
                }
            }
            else
            {
                ...
                return FindObjectInHierarchyRecursively(((GameObject)rootAsset).transform, pathComponents, 0,
                    referenceType);
            }
        ...
        private Object FindObjectInHierarchyRecursively(Transform obj, string[] path, int pathIndex, Type targetType)
        {
            ...
            if (obj.GetComponent<TMP_InputField>())
            {
                ...
                if (inputFieldViewport)
                {
                    ...
                    {
                        Object result = FindObjectInHierarchyRecursively(inputFieldViewport.GetChild(i), path,
                            pathIndex + 1, targetType);
                        if (result)
                            return result;
                    }
                }
